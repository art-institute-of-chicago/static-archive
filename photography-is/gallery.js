...
function setupTooltips(map_container, orientation, attempts) {
  ...
  if($(map_container).find('.fleximap-area').length > 0) {
    $(map_container).find('.fleximap-area').each(function() {
      var tip = '<p>' + 
                '<span class="tip-artist">' + $(this).data('artist') + '</span>' +
                '<span class="tip-period">' + $(this).data('period') + '</span>' +
                '<span class="tip-title">' + $(this).data('title') + '</span>' +
                '<span class="tip-date">' + $(this).data('date') + '</span>' +
                '<span class="tip-medium">' + $(this).data('medium') + '</span>' +
                '<span class="tip-collection">' + $(this).data('collection') + '</span>' +
                '</p>',
          tipside = $(this).data('tipside') || orientation;
...
function createSingleModal(modalData) {
  ...
  var modal = ' \
    <div id="' + modalId + '" class="reveal-modal gallery"> \
      ...
      <p class="modal-details">' + modalData["date"] + '</p> \
...
function addClickTextToTooltip(area) {
  $(area).attr('title', $(area).attr('title') + '<p class="click-tip">Click this artwork for more information</p>');
}
