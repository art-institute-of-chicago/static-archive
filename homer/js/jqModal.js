...
open:function(s,t){var h=H[s],c=h.c,cc='.'+c.closeClass,z=/^\d+$/.test(h.w.css('z-index'))&&h.w.css('z-index')||c.zIndex,o=$('<div></div>').css({height:$(document).height(),width:$(document).width(),position:'fixed',left:0,top:0,'z-index':z-1,opacity:c.overlay/100});h.t=t;h.a=true;h.w.css('z-index',z);
 ...
 // IF IE6;
	 ...
	 //alert (ie6 + " " + jQuery.browser.msie );
	  
 //  Set the Overlay to 100% height/width, and fix-position it via JS workaround
if(jQuery.browser.msie&&o)
{
	o=o.css({position:'absolute',height:'100%',width:'100%'})[0];for(var y in {Top:1,Left:1})o.style.setExpression(y.toLowerCase(),"(_=(document.documentElement.scroll"+y+" || document.body.scroll"+y+"))+'px'");
}
